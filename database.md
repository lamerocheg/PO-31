...
# Microsoft PLSQL Server

Имя сервера: VAK-DB\PLSQLExpress
Имя базы: StudTraining

#### Из Access в MS PLSQL Server
`
Вкладка Таблицы -> Export -> (Тип данных: ODBC-источник) -> mdb -> 
Создать -> Выбрать сервер -> VAK-DB\PLSQLExpress -> StudTraining
`

## Расширения языка PLSQL в MS PLSQL Server
...
Или так (С помощью UPDATE FROM в MS PLSQL):
...
Или так, чтобы информация с NULL коректно обновлялась (UPDATE FROM в MS PLSQL)
...
# Создание таблиц в MS-PLSQL сервер
...
Для MS PLSQL представление может содержать перемножение таблиц, но в операторах обновления должны использоваться поля только одной таблицы.

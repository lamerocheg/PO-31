...
# Microsoft PLpgSQL Server

Имя сервера: VAK-DB\PLpgSQLExpress
Имя базы: StudTraining

#### Из Access в MS SQL Server
`
Вкладка Таблицы -> Export -> (Тип данных: ODBC-источник) -> mdb -> 
Создать -> Выбрать сервер -> VAK-DB\PLpgSQLExpress -> StudTraining
`
...
# Программное формирование и выполнение операторов PLpgSQL

Это может быть необходимо потому как параметры процедур не могуть быть использованы как имена таблиц или имена полей в PLpgSQL операторах.
